...
def run_simu_robust():
    ...
    for cur_arr in optimal_result :
        for i_node in cur_arr:
            for j_node in np.setdiff1d(np.arange(N),cur_arr):
                if G_optimal.has_edge(int(i_node), j_node) and np.random.binomial(1,p = fail_p) == 0:
    ...
    for cur_arr in health_result:
        for i_node in cur_arr:
            for j_node in np.setdiff1d(np.arange(N),cur_arr):
                if G_health.has_edge(int(i_node), j_node) and np.random.binomial(1,p = fail_p):
    ...
    for cur_arr in vacc_result:
       for i_node in cur_arr:
           for j_node in np.setdiff1d(np.arange(N),cur_arr):
               if G_vacc.has_edge(int(i_node), j_node) and np.random.binomial(1,p = fail_p):
    ...
    for cur_arr in obe_result:
       for i_node in cur_arr:
           for j_node in np.setdiff1d(np.arange(N),cur_arr):
               if G_obe.has_edge(int(i_node), j_node) and np.random.binomial(1,p = fail_p):
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = [executor.submit(run_simu_robust) for _ in range(num_exp)] 
    ...
    np.savez('results_pf_'+str(f_type)+'_graph_robust_'+str(graph_type)+'.npz', simulation_result = simulation_result, simulation_edges = simulation_edges)
    ...
    results = np.load('results_pf_'+str(f_type)+'_graph_robust_'+str(graph_type)+'.npz')
    ...
    fig, ax = plt.subplots()
    ax.plot(range(time_int), simulation_mean[1,:],'k-' , linewidth=1 ,label = 'ISSE')
    ax.fill_between(range(time_int), lower[1,:], upper[1,:], color='k', alpha=.2,label = '95% confidence interval')
    ax.plot(range(time_int), simulation_mean[2,:],'k--' , linewidth=1 ,label = 'HB-P')
    ax.fill_between(range(time_int), lower[2,:], upper[2,:], color='k', alpha=.2)
    ax.plot(range(time_int), simulation_mean[3,:],'k-.' , linewidth=1 ,label = 'VB-P')
    ax.fill_between(range(time_int), lower[3,:], upper[3,:], color='k', alpha=.2)
    ax.plot(range(time_int), simulation_mean[4,:],'k:' , linewidth=1 ,label = 'OB-P')
    ...
    plt.savefig('Simulation_pf_' + str(f_type) + '_graph_robust_' + str(graph_type)+'.pdf')
    plt.show()
